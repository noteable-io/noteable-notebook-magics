...
def test_post(api, mock_success):
    with mock.patch.object(api._session, 'request', return_value=mock_success) as mock_request:
...
def test_fs_pull(fs, mock_success):
    with mock.patch.object(fs._api._session, 'request', return_value=mock_success) as mock_request:
